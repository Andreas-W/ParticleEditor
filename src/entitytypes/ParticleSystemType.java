...
public class ParticleSystemType {
	...
	public ParticleSystemType(ParticleSystemType other) {
		...
		VolBoxHalfSize = other.VolBoxHalfSize.clone();
	...
	public float[] ColorScale = new float[2];
	...
	public String createInnerCode() {
		...
		if (PerParticleAttachedSystem != null && !PerParticleAttachedSystem.equals("")) {
			sb.append("PerParticleAttachedSystem = "+PerParticleAttachedSystem+"\n");
		}
		if (SlaveSystem != null && !SlaveSystem.equals("")) {
			sb.append("SlaveSystem = "+SlaveSystem+"\n");
			if (SlavePosOffset != null && !SlavePosOffset.isZero()) {
				sb.append("SlavePosOffset"+SlavePosOffset.getCode());
			}
		}
		...
		if (Util.hasValues(ColorScale)) sb.append(String.format("ColorScale = %s %s\n", Util.fmt(ColorScale[0]), Util.fmt(ColorScale[1])));
		...
		if (DriftVelocity!= null && !DriftVelocity.isZero()) sb.append("DriftVelocity").append(DriftVelocity.getCode());
	...
	public void setValues(ParticleSystemType other) {
		Priority = other.Priority;
		IsOneShot = other.IsOneShot;
		Shader = other.Shader;
		Type = other.Type;
